 ...
 */    
public class OutlierNodeDetect_Out extends Thread{  
	/*k表示第几邻近*/
    ...
    public void run()
    {
    	...
    	try
    	{
    		...
    		while(rs0.next())
    		{
    			float longitude = rs0.getFloat(9);
    			float latitude = rs0.getFloat(10);
    			float[] i = new float[2];   i[0] = longitude;   i[1] = latitude;
    			id[count] = rs0.getString(1);   time[count] = rs0.getString(8);   count++;
    			list.add(i);
    		}
    		OutlierNodeDetect_Out A = new OutlierNodeDetect_Out();
    		String end=A.progress(list);
    		System.out.println(end);
    		
    		for(int j=0; j<count; j++)
    		{
    			String outresult = end.substring(j, j+1);
    			
    			SimpleDateFormat sd1 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
            	ParsePosition pos1 = new ParsePosition(0);
        		Date d2 = (Date) sd1.parse(time[j], pos1);        		
        		long interval = d1.getTime() - d2.getTime();// 得出的时间间隔是毫秒
        		if(interval >= 0) outresult = "N";//学生签到时间在老师之前，则签到失败
        		
    			String cur_id = id[j];
    			String sql = "update AttendanceInfo set OutResult='" + outresult + "' where StudentID='" + cur_id +"' and CourseNum='"+CourseNum+"' and OutTime like '" + Time + "'";
    ...
    public String progress (ArrayList<float[]> list) 
    {
        ...
        {
        	OutlierNodeDetect_Out lof = new OutlierNodeDetect_Out(); 
